...
if(isset($_POST['adminLogin'])) {
    ...
    if(count($errors) == 0) {
        $pass = md5($password);
        $query = "SELECT * FROM admins WHERE admin_email='$email' AND admin_pass='$pass'";
        $result = mysqli_query($link,$query);

        $sql= "SELECT * FROM admins WHERE admin_email='$email'";
        $res = mysqli_query($link,$sql);
        $admin_id = mysqli_fetch_assoc($res)['admin_id'];

        if(mysqli_num_rows($result) == 1) {
            $username = mysqli_fetch_assoc($result)['admin_name'];
            $_SESSION['id'] = $admin_id;
            $_SESSION['username'] = $username;
            $_SESSION['type'] ='admin';
            $_SESSION['success'] = 'Login successful';
...
if(isset($_POST[''])) {
    ...
    $fromdate = mysqli_real_escape_string($link, $_POST['fromdate']);
     $todate = mysqli_real_escape_string($link, $_POST['todate']);
      $price = mysqli_real_escape_string($link, $_POST['message']);
    ...
    if(count($errors) == 0) {
        $sql = "INSERT INTO bookings (fullname,fromdate,todate,message) VALUES ('$name','$fromdate','$todate','$price')";
        mysqli_query($link,$sql);
